    ...
    def __init__(self, in_channels, out_channels=1024, aspect_ratios=[[1]], scales=[1], parent=None, index=0,
        ...
        if parent is None:
            if cfg.extra_head_net is None:
                ...
                self.upfeature, out_channels = make_net(in_channels, cfg.extra_head_net, activation_func=self.acf)
...
class FPN(ScriptModuleWrapper):
    ...
    """
    __constants__ = ['interpolation_mode', 'num_downsample', 'use_conv_downsample', 'acf_pred_layers',
                     'lat_layers', 'pred_layers', 'downsample_layers', 'acf_downsample_layers']
    ...
    def __init__(self, in_channels):
        ...
        self.acf_downsample_layers = cfg.fpn.acf_downsample_layers
        self.acf_pred_layers = cfg.fpn.acf_pred_layers
    ...
    def forward(self, convouts: List[torch.Tensor]):
        ...
        for pred_layer in self.pred_layers:
            ...
            if self.acf_pred_layers:
                F.acf(out[j], inplace=True)
        ...
        if self.acf_downsample_layers:
            for idx in range(len(out) - cur_idx):
                out[idx] = F.acf(out[idx + cur_idx], inplace=False)
...
class FastMaskIoUNet(ScriptModuleWrapper):
    ...
    def __init__(self):
        ...
        self.maskiou_net, _ = make_net(input_channels, cfg.maskiou_net + last_layer, include_last_acf=True,
                                       activation_func=cfg.activation_fuc)
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        if cfg.mask_type == mask_type.direct:
            ...
            self.proto_net, cfg.mask_dim = make_net(in_channels, cfg.mask_proto_net, include_last_acf=False,
